...
def main():
    ...
    cached_count = len(unique_domains) - len(domains_to_check)
    logging.info(f"Using {cached_count} cached results, checking {len(domains_to_check)} domains")
    ...
    if domains_to_check:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {executor.submit(check_domain, domain, resolver): domain for domain in domains_to_check}
